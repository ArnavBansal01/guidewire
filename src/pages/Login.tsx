...
const Login = () => {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      if (userSnap.exists()) {
        ...
        // They authenticated with Google but never finished the registration form.
        // Send them to the register page so they can add their City/Platform.
        navigate('/register');
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-[calc(100vh-4rem)] flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="max-w-md w-full bg-white dark:bg-slate-900/50 backdrop-blur-sm p-8 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800">
        ...
        {/* Developer Backdoor: Temporary Quick Access */}
        <div className="mt-6 pt-6 border-t border-slate-200 dark:border-slate-700">
          <button
            onClick={() => navigate('/dashboard')}
            className="w-full px-4 py-3 bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-white font-medium rounded-lg hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors border border-slate-200 dark:border-slate-700"
          >
