...
import { Shield, UserPlus, ArrowRight, UserCheck, ShieldAlert } from 'lucide-react';
...
const Login = () => {
  ...
  const handleDemoMode = (role: "worker" | "admin") => {
    setDemoState({ running: true, message: `Entering Phone ${role === 'worker' ? '9999999999' : '8888888888'}...` });
    setTimeout(() => {
      setDemoState({ running: true, message: `Entering OTP ${role === 'worker' ? '1234' : '4321'}...` });
      setTimeout(() => {
      ...
      <div className="hidden lg:flex w-1/2 relative bg-slate-950 items-center justify-center p-12 lg:p-20 overflow-hidden border-r border-slate-800/60 z-10">
        ...
        <div className="relative z-10 max-w-lg w-full">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-cyan-500/10 border border-cyan-500/20 mb-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <span className="text-xs font-black text-cyan-300 uppercase tracking-widest">System Access</span>
          </div>
          ...
          <p className="text-lg text-slate-400 font-medium leading-relaxed animate-in fade-in slide-in-from-bottom-8 duration-700 delay-200">
            Welcome back to GigShield. Instantly monitor risk triggers, manage your dynamic payouts, and secure your daily earnings.
          </p>
        </div>
      </div>
        ...
        <div className="w-full max-w-md flex flex-col relative z-10">
          ...
          <div className="lg:hidden mb-8 text-center animate-in fade-in slide-in-from-top-4 duration-700">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-cyan-500/10 border border-cyan-500/20 mb-4 shadow-sm">
              ...
              <span className="text-[10px] font-black text-cyan-700 dark:text-cyan-400 uppercase tracking-widest">System Access</span>
            </div>
          ...
          {needsRegistration ? (
            ...
            <div className="animate-in fade-in duration-300">
              ...
              <p className="text-center text-sm text-slate-600 dark:text-slate-400 mt-6 mb-2">
                No account?{' '}
                <Link to="/register" className="font-semibold text-cyan-600 hover:text-cyan-500 dark:text-cyan-400 dark:hover:text-cyan-300">
                  Register here
                </Link>
              </p>
            </div>
          ...
          {demoState.running && (
            <div className="absolute inset-0 z-50 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md flex flex-col items-center justify-center text-center animate-in fade-in duration-300 rounded-[32px]">
              ...
              <p className="text-xl font-bold text-slate-900 dark:text-white mb-2">{demoState.message}</p>
              <p className="text-sm text-slate-500 font-medium">Bypassing standard authentication for judges...</p>
            </div>
          )}
          ...
          <div className="mt-8 pt-8 border-t border-slate-200 dark:border-slate-800 relative z-10">
            <div className="mb-5 text-center">
              <span className="text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Demo Access (Judges)</span>
            </div>
